...
exports.UpdatedeliveryNote = async (req, res) => {
  ...
  const updateDeliveryNoteSql = `
      UPDATE deliveryNote 
      SET deliveryNoteNumber = ?, deliveryDate = ?, deliveryNoteStatus = ?, branchId = ?
      WHERE deliveryNoteId = ?`;
  const updateDeliveryNoteValues = [deliveryNoteNumber, deliveryDate, deliveryNoteStatus, branchId, deliveryNoteId];
  
  try {
      await dbConnection.query(updateDeliveryNoteSql, {
          replacements: updateDeliveryNoteValues,
